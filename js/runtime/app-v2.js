(() => {
  ...
  const fx=(kind,detail)=>window.RED_FLAG_AUDIO?.playSfx?.(kind,detail);
  function replayClass(node,name,ms=620){if(!node)return;node.classList.remove(name);void node.offsetWidth;node.classList.add(name);setTimeout(()=>node.classList.remove(name),ms);}
  function showUnlock(title,text){let stack=document.querySelector('.rfd-unlock-stack');if(!stack){stack=document.createElement('div');stack.className='rfd-unlock-stack';document.body.appendChild(stack);}const note=document.createElement('div');note.className='rfd-unlock-note';note.innerHTML=`<small>DETECTED FILE UNLOCKED</small><b>${title}</b><span>${text}</span>`;stack.appendChild(note);requestAnimationFrame(()=>note.classList.add('show'));setTimeout(()=>{note.classList.remove('show');setTimeout(()=>note.remove(),220);},2600);}
  ...
  function probeContext(item){const text=`${item.type||''} ${item.quote||''}`;
    ...
    if(/明天.*補|再補.*約|改天|下次|重新約|補約/.test(text))return{choice:'先把補約講成具體安排，再決定怎麼回',question:'你說要補約，現在能不能先把時間和安排說清楚？',result:'妳沒有把一句「下次再約」直接當成承諾，先確認他會不會把安排落實。'};
    if(/累|不想|不要|拒絕|不舒服|繼續盧|停下|尊重/.test(text))return{choice:'先確認他是否真的尊重妳的拒絕，再決定怎麼回',question:'你剛剛停下來，是理解我的狀態，還是其實還有不舒服？',result:'妳肯定他有停下來，也把彼此的感受留在可以說清楚的位置。'};
  ...
  function renderRound(){
    ...
    const choices=$('choices');choices.innerHTML='';getOptions(item).forEach((choice,i)=>{const effective=modifierDelta(choice.delta);const b=document.createElement('button');b.type='button';b.dataset.choice=i;b.className=choice.special?'unlocked-choice':choice.probe?'case-tool-choice':'';b.innerHTML=`<b>${['A','B','C','D'][i]}｜${choice.text}</b><small>${choice.note}</small><small class="impact">${impactText(effective)}</small>`;b.onclick=()=>choose(choice,item,b,i,effective);choices.appendChild(b);});replayClass($('game'),'round-enter',500);if(item.kind==='event')fx('event');else if(item.rare||item.special)fx('rare');updateProgress();
  }
  ...
  function persistDiscovery(item){try{const d=JSON.parse(localStorage.getItem('rfd-dex')||'{"personas":{},"events":{},"rare":{}}');let unlocked=null;if(item.kind==='event'){if(!d.events[item.id])unlocked={title:item.title,text:item.type};d.events[item.id]={title:item.title,type:item.type};}else{if(!d.personas[item.persona])unlocked={title:personas[item.persona]?.role||'新人物檔案',text:'人物資料已收錄'};d.personas[item.persona]={role:personas[item.persona]?.role||'',profile:personas[item.persona]?.profile||''};if((item.rare||item.special)&&!d.rare[item.id])unlocked={title:personas[item.persona]?.name||'RARE FILE',text:item.type};if(item.rare||item.special)d.rare[item.id]={name:personas[item.persona]?.name||'',type:item.type};}localStorage.setItem('rfd-dex',JSON.stringify(d));return unlocked;}catch{return null;}}
