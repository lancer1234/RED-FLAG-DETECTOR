(() => {
  ...
  const map=new Map((window.RED_FLAG_DATA||[]).filter(x=>/^P1[3-9]-|^P20-/.test(x.id)).map(x=>[String(x.quote||'').trim(),x.persona]));
  ...
  function decorate(){
    ...
    if(id==='P19'){
      // timeline / handoff: two overlapping bars with a tiny arrow
      px(ctx,148,102,32,6,red);px(ctx,163,113,31,6,cyan);px(ctx,174,106,4,13,gold);
      px(ctx,178,108,7,3,gold);px(ctx,178,114,7,3,gold);
    }
    if(id==='P20'){
      // family DLC: house + chat bubble
      px(ctx,149,103,30,22,gold);px(ctx,153,96,22,8,red);px(ctx,159,110,10,15,dark);
      px(ctx,181,98,24,15,cyan);px(ctx,186,113,5,5,cyan);px(ctx,185,102,16,3,dark);px(ctx,185,107,12,3,dark);
    }
  }
